  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stats.isDirectory()) {
        ...
        if (['.bin', 'node_modules', '.git', 'test', 'tests', '__tests__', 'docs', 'examples'].includes(file)) {
          continue;
        }
...
function generateRecommendations(deps, imports) {
  ...
  const iconImports = imports.namedImports.filter(i => 
    i.module.includes('lucide-react-native') || 
    i.module.includes('@expo/vector-icons')
  );
...
async function analyzeBundle() {
  ...
  if (options.json) {
    ...
    console.log(`Heavy deps (>500KB): ${deps.filter(d => d.size > SIZE_THRESHOLDS.large).length}`);
    ...
    deps.slice(0, 10).forEach((dep, i) => {
      ...
      console.log(`${(i + 1).toString().padStart(2)}. ${color}${formatBytes(dep.size).padStart(10)}\x1b[0m ${dep.name}`);
    });
