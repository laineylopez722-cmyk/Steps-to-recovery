...
export function measureSync<T>(operationName: string, operation: () => T): T {
  ...
  try {
    ...
    if (duration > 100) {
      logger.warn(`Heavy sync operation: ${operationName} took ${duration.toFixed(2)}ms. Consider offloading to async.`);
    }
...
export function logMemoryUsage(): void {
  ...
  if (memory.usedJSHeapSize !== null) {
    ...
    const totalMB = memory.totalJSHeapSize ? (memory.totalJSHeapSize / 1024 / 1024).toFixed(2) : 'N/A';
    const limitMB = memory.jsHeapSizeLimit ? (memory.jsHeapSizeLimit / 1024 / 1024).toFixed(2) : 'N/A';
