...
function EntryCard({ entry, onPress }: { entry: JournalEntryDecrypted; onPress: () => void }) {
...
function EmptyState({ isSearching }: { isSearching: boolean }) {
  ...
  const ds = useDs();
  return (
    <Animated.View entering={MotionTransitions.fadeDelayed(200)} style={styles.empty}>
      <Feather
      ...
      <Text style={styles.emptyText}>
        {isSearching
          ...
          : "Your first entry is waiting. What's on your mind today?"}
      </Text>
    </Animated.View>
  );
}
...
export function JournalListScreen({ userId }: Props): React.ReactElement {
  ...
  const groupedEntries = useMemo(() => {
    ...
    if (thisWeekEntries.length > 0)
      groups.push({ title: 'Previous 7 Days', data: thisWeekEntries });
  ...
  const renderItem = useCallback(({ item, index }: { item: FlatListItem; index: number }) => {
    ...
    return (
      <Animated.View entering={MotionTransitions.cardEnter(index)} layout={Layout.springify()}>
        <EntryCard entry={item.data} onPress={() => handleEntryPress(item.data)} />
      </Animated.View>
    );
  }, []);
    ...
    <View style={styles.container}>
      <SafeAreaView style={styles.safe} edges={['top']}>
        ...
        <FlashList
          ...
          ListEmptyComponent={!isLoading ? <EmptyState isSearching={!!search} /> : null}
