...
export function AmendsTracker({ entries, onAdd, onUpdate }: AmendsTrackerProps) {
  ...
  const toggleStatus = async (entry: AmendsEntry) => {
    const nextStatus = entry.status === 'not_started' ? 'in_progress' 
      : entry.status === 'in_progress' ? 'complete' 
      : 'not_started';
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            <View className="flex-1">
              ...
              <Text className="text-gray-400 text-sm" numberOfLines={1}>{item.harm}</Text>
        ...
        <View className="absolute inset-0 bg-black/95 p-4">
          ...
          <View className="flex-row flex-wrap gap-2 mb-6">
            {AMENDS_TYPES.map(type => (
              <TouchableOpacity
                key={type.id}
                onPress={() => setNewEntry(p => ({ ...p, amendsType: type.id as AmendsEntry['amendsType'] }))}
              ...
              >
                <Text className={newEntry.amendsType === type.id ? 'text-amber-500' : 'text-gray-400'}>
                  {type.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View className="flex-row gap-3">
            ...
            >
              <Text className={`text-center font-medium ${newEntry.who && newEntry.harm ? 'text-black' : 'text-gray-500'}`}>
                Add
              </Text>
            </TouchableOpacity>
          </View>
