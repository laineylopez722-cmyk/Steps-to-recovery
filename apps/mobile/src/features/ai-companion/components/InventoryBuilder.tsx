...
export function InventoryBuilder({ onSave, onCancel, initialEntry }: InventoryBuilderProps): React.ReactElement {
  ...
  const canProceed = (): boolean => {
    switch (step) {
      ...
      case 3: return (entry.affects?.length || 0) > 0;
      case 4: return !!entry.myPart?.trim();
    ...
    <View className="flex-1 bg-black p-4">
          ...
          <View className="gap-2">
            {AFFECTS_OPTIONS.map(option => (
              <TouchableOpacity
                ...
                `}
              >
                <Text className={entry.affects?.includes(option.id) ? 'text-amber-500' : 'text-white'}>
                  {option.label}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      {step === 4 && (
        <View>
          ...
          <Text className="text-gray-400 mb-4">What was your role? Where were you selfish, dishonest, self-seeking, or frightened?</Text>
          <TextInput
            value={entry.myPart}
      ...
      <View className="flex-row gap-3 mt-auto pt-4">
        ...
        >
          <Text className={`text-center font-medium ${canProceed() ? 'text-black' : 'text-gray-500'}`}>
