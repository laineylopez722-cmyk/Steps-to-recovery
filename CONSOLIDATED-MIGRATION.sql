...
-- Migrations included:
--   1. Reading Reflections (JFT feature)
--   2. Day Rating (check-in enhancement)
--   3. Safe Dial Tables (crisis intervention)
--   4. Sponsor Notifications (risk detection)
--   5. Meeting Reflections (pre/post prompts)
--   6. Meeting Check-ins (90 in 90 feature)
-- =====================================================
...
COMMENT ON TABLE reading_reflections IS 'User reflections on JFT daily readings';
...
BEGIN
  IF NOT EXISTS (
    ...
    COMMENT ON COLUMN daily_checkins.day_rating IS 'Overall day rating (1-10, optional)';
  END IF;
END
$$;
...
COMMENT ON TABLE meeting_checkins IS 'Meeting attendance tracking for 90 in 90 and achievements';
...
COMMENT ON TABLE user_achievements IS 'Unlocked achievement badges for users';
...
BEGIN
  ...
  RAISE NOTICE '  - reading_reflections';
  RAISE NOTICE '  - daily_checkins (day_rating column)';
  RAISE NOTICE '  - meeting_checkins';
  RAISE NOTICE '  - user_achievements';
  RAISE NOTICE '  - risky_contacts';
  RAISE NOTICE '  - close_calls';
  RAISE NOTICE '  - sponsor_notifications';
  RAISE NOTICE '  - meeting_reflections';
END $$;
